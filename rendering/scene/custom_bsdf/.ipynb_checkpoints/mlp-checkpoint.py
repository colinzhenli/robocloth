...
from custom_bsdf.material.anisotropicLatent import AnisotropicLatentTexturedModel,MERLBRDF,MERLInterface,HyperBRDF,MerlTorch
from custom_bsdf.material.isotropicLatent import LatentTexturedModel
from custom_bsdf.material.svpbr import SvPBRBRDF
...
class MLPBRDF(mi.BSDF):
    def __init__(self, props):
        ...
        if props.has_property('model_path'):
            ...
            model_path = props.string('model_path', '')  # Provide default value
            ...
            material_type = props.string('material_type', '')
        ...
        elif props.has_property('use_anisotropic') and props.bool('use_anisotropic', False):
    ...
    def sample(self, ctx, si, sample1, sample2, active):
        ...
        print("sample2_torch",sample2_torch)
    ...
    def eval(self, ctx, si, wo, active):
        ...
        return dr.select((cos_theta_i > 0) & (cos_theta_o > 0), value, mi.Vector3f(0.0))
    ...
    def eval_anisotropic_mlp(self, si, wo):
        ...
        pos_torch = si.p.torch().detach()
        
        # 2. Geometric normal (world coordinate) - create independent copy
        normal_torch = NF.normalize(si.sh_frame.n.torch().detach(), dim=-1)
        #print("normal_torch",normal_torch)
        tangent_torch = NF.normalize(si.sh_frame.s.torch().detach(), dim=-1)

        bitangent_torch = NF.normalize(si.sh_frame.t.torch().detach(), dim=-1)
        ...
        uv_torch = si.uv.torch().detach()
        uv_torch=(uv_torch*3.0)%1.0
        ...
        with torch.no_grad():
            #brdf, _,_ = self.anisotropic_model.eval_brdf({}, pos_torch.cuda(), wo_torch.cuda(), wi_torch.cuda(), normal_torch.cuda(),uv_torch.cuda(), TBN_torch.cuda())
            brdf = self.anisotropic_model.eval_brdf(wo_torch.cuda(), wi_torch.cuda(),material_ids,weights)
        '''
        ...
        result = mi.Vector3f(
            dr.cuda.Float(brdf[:, 0]),
            dr.cuda.Float(brdf[:, 1]), 
            dr.cuda.Float(brdf[:, 2])
        )
