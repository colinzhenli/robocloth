...
class Merl:
    ...
    def from_array(self, brdf_arr) -> object:
    ...
    def to_dataframe(self, mode='interpolation', angles_only=False):
        cols = []
    ...
    def eval_interp(self, theta_h, theta_d, phi_d):
        ...
        print("angle",phi_d,theta_d,theta_h)
        print("idx",idx_pd_p,idx_td_p,idx_th_p)
        ...
        # Print the number of values less than 0
        '''
        print("size of ret_val",len(ret_val[0]))
        print(f"Number of positive values: {(np.array(ret_val) > 0).sum()}")
        print(f"Number of negative values: {(np.array(ret_val) < 0).sum()}")
        
        ret_val = np.array(ret_val)
        negative_values = ret_val[ret_val < 0]
    ...
    def __eval_idx(self, ith, itd, ipd):
        ...
        for color in range(0, 3):
            ret += [self.brdf_np[(ind + color * stride).astype(np.int32)] * self.scale[color]]
