...
# ============================================================================
class MipmapLatentTexture(nn.Module):
    """
    Mipmap latent texture storing a pyramid of latent grids.
    
    Two modes:
        prefilter=True:  Single finest-level learnable texture; coarser levels
                         are built on-the-fly by area downsampling.
        prefilter=False: Separate learnable texture per mipmap level, each
    ...
    def __init__(
        ...
        if blur_config is not None:
            self.blur_sigma0 = blur_config.get('blur_sigma0', 2.0)
            self.blur_half_life = blur_config.get('blur_half_life', 3333)
        else:
            self.blur_sigma0 = 2.0
            self.blur_half_life = 3333
        ...
        if self.prefilter:
            ...
            for level in range(mipmap_levels):
                level_res = max(1, resolution // (2 ** level))
                latent_init = torch.randn(1, latent_dim, level_res, level_res) * init_std
                if predict_frame:
                    latent_init[:, -6:-3, :, :] = torch.tensor([0.0, 0.0, 1.0]).view(1, 3, 1, 1)
                    latent_init[:, -3:, :, :] = torch.tensor([0.0, 1.0, 0.0]).view(1, 3, 1, 1)
                self.level_params.append(nn.Parameter(latent_init))
    
    def _gaussian_kernel(self, sigma: float, channels: int, ref_tensor: torch.Tensor):
        """Generate Gaussian kernel for depth-wise convolution."""
        if sigma < 0.5:
            return None
        radius = int(math.ceil(3 * sigma))
    ...
    def _apply_blur(self, tex: torch.Tensor, sigma: float):
        """Apply Gaussian blur to a single [1, C, H, W] texture."""
        kernel = self._gaussian_kernel(sigma, tex.shape[1], tex)
        if kernel is None:
            return tex
        pad = kernel.shape[-1] // 2
        return NF.conv2d(tex, kernel, padding=pad, groups=tex.shape[1])
    
    def _build_pyramid(self, base_tex: torch.Tensor):
        """Downsample base_tex into a mipmap pyramid via area filtering.
        
        Args:
            base_tex: [1, C, H, W] finest-level texture
        Returns:
            list of mipmap_levels tensors, each [1, C, H_l, W_l]
        """
        pyramid = [base_tex]
        H, W = base_tex.shape[2], base_tex.shape[3]
        for level in range(1, self.mipmap_levels):
            new_h = max(1, H // (2 ** level))
            new_w = max(1, W // (2 ** level))
            downsampled = NF.interpolate(base_tex, size=(new_h, new_w), mode='area')
            pyramid.append(downsampled)
        return pyramid
    
    def get_mipmap_textures(self, step: int = None, apply_blur: bool = False):
        """Return the mipmap pyramid as a list of [1, C, H_l, W_l] tensors.
        
        Args:
            step: Current training step (used for progressive blur schedule).
            apply_blur: Whether to apply Gaussian blur (typically True during training).
        Returns:
            list of mipmap_levels tensors
        """
        sigma = self.blur_sigma0 * (0.5 ** (step / self.blur_half_life)) if (apply_blur and step is not None) else 0.0
        
        if self.prefilter:
            base_tex = self._apply_blur(self.params, sigma) if sigma >= 0.5 else self.params
            return self._build_pyramid(base_tex)
        else:
            if sigma >= 0.5:
                return [self._apply_blur(p, sigma) for p in self.level_params]
            else:
                return list(self.level_params)
    
    def get_finest_texture(self, step: int = None, apply_blur: bool = False):
        """Return only the finest-level texture [1, C, H, W]."""
        sigma = self.blur_sigma0 * (0.5 ** (step / self.blur_half_life)) if (apply_blur and step is not None) else 0.0
        if self.prefilter:
            return self._apply_blur(self.params, sigma) if sigma >= 0.5 else self.params
        else:
            return self._apply_blur(self.level_params[0], sigma) if sigma >= 0.5 else self.level_params[0]
...
# ============================================================================
class MultiMaterialPBRLatentBRDF(LightningModule):
    """PBR sanity-check variant of MultiMaterialLatentBRDF.

    Same per-material training-list metadata + per-point latent bank, but the
    decoder is the analytical PBRDecoder instead of a learned MLP.

    Latent layout (per point):
        [brdf_params(brdf_latent_dim), normal(3), tangent(3)]
    where brdf_latent_dim is fixed by the (anisotropic, disney) flags:
        isotropic  -> 6  [color(3), albedo(1), roughness(1), metallic(1)]
        anisotropic-> 9  + [ior(1), aniso_strength(1), aniso_rot(1)]
        disney     -> 12 + [specularTint(1), sheen(1), sheenTint(1)]
    """
    ...
    def __init__(self, cfg):
        ...
        self.predict_frame = getattr(cfg, 'predict_frame', True)
        ...
        # Always reserve 6 dims for normal+tangent (multi-material has no GT
        # frame, so predict_frame is effectively required).
        self.total_latent_dim = self.brdf_latent_dim + 6

        # Expose latent_dim to be compatible with trainer code that slices
        # latents as [:, :material.latent_dim].
        self.latent_dim = self.brdf_latent_dim

        # Optimizer / sparse-embedding config (mirrors MultiMaterialLatentBRDF)
        self.optimizer_name = getattr(cfg, 'optimizer', {}).get('name', 'Adam')
        # PBR always dense — the latent dims are small (6/9/12) and there is
        # no encode_directions path, so sparse embeddings buy nothing.
        is_sparse = False

        # Point subsampling ratio — must match cfg.data.point_subsample_ratio
        self.point_subsample_ratio = float(getattr(cfg, 'point_subsample_ratio', 1.0))
        ...
        # PBRDecoder is analytical — no trainable parameters
        self.decoder = PBRDecoder(
            cfg=cfg,
            soft_constraint=self.soft_constraint,
        )

        # No smooth_reg for analytical PBR
        self.smooth_reg = False

        print("MultiMaterialPBRLatentBRDF initialization complete!")

    # Reuse MultiMaterialLatentBRDF's metadata loader verbatim (training_list
    # + per-material point_metadata.json + optional legacy_swap_indexing).
    _load_point_metadata = MultiMaterialLatentBRDF._load_point_metadata
    ...
    def sample_brdf(
        ...
        # Cosine-weighted hemisphere sampling in world frame (same placeholder
        # as MultiMaterialLatentBRDF; brdf_sampling branch isn't exercised in
        # the points_dense stage-1 path).
        theta = torch.asin(torch.sqrt(sample2[..., 0]))
        phi = 2 * math.pi * sample2[..., 1]
